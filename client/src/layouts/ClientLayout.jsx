    ...
    <AnimatePresence>
      {!collapsed ? (
        ...
        <motion.div key="collapsed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <Tooltip label={user?.name || "Client"} show>
...
// ─── Client Navbar ─────────────────────────────────────────────────────────
const ClientNavbar = ({ onSearch, mobileOpen, setMobileOpen }) => {
  ...
  const currentConfig = CLIENT_CONFIG[location.pathname] || { title: "Overview", icon: LayoutDashboard };
                ...
                <motion.div initial={{ opacity: 0, scale: 0.96, y: -8 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                    ...
                    <div className="py-2 px-2 space-y-1">
                      <motion.button whileHover={{ x: 2 }}
                        onClick={() => { navigate("/client/dashboard"); setDropdownOpen(false); }}
          ...
          {NAV_SECTIONS.map((section, si) => (
            <div key={section.label}>
              ...
              <div className="space-y-[2px]">
                {section.items.map(item => <NavItem key={item.to} {...item} collapsed={collapsed} onClick={() => setMobileOpen(false)} />)}
              </div>
            </div>
          ))}
        </nav>
