...
import { Zap, ArrowRight, Github, Eye, EyeOff, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex bg-surface-secondary dark:bg-dark-bg">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] bg-navy p-12 relative overflow-hidden">
        ...
        <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-cyan/10 blur-3xl" />
        ...
        <div className="relative z-10 space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <h2 className="text-3xl font-bold text-white leading-tight">
              ...
              <span className="text-gradient-cyan">business smarter.</span>
            </h2>
            <p className="text-slate-400 mt-3 text-base leading-relaxed">
              Projects, clients, invoices, and AI assistance — all in one place.
            </p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
            ...
            <div className="flex -space-x-2">
              {["#635BFF","#10B981","#F59E0B","#EF4444"].map((c, i) => (
                <div key={i} className="w-7 h-7 rounded-full border-2 border-navy flex items-center justify-center text-xs text-white font-bold"
                  style={{ background: c }}>{String.fromCharCode(65 + i)}</div>
              ))}
            </div>
            <div>
              <p className="text-white text-sm font-medium">2,400+ freelancers</p>
              <p className="text-slate-400 text-xs">trust Skillora daily</p>
            </div>
          </motion.div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-ink dark:text-slate-100 mb-1">Welcome back</h1>
          <p className="text-sm text-ink-secondary mb-7">Sign in to your account to continue</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input type={showPw ? "text" : "password"} name="password" placeholder="••••••••"
                value={form.password} onChange={handleChange} error={errors?.password} required size="lg"
            ...
            <label className="flex items-center gap-2.5 cursor-pointer select-none">
              <input type="checkbox" name="rememberMe" checked={form.rememberMe} onChange={handleChange}
                className="w-4 h-4 rounded border-surface-border text-brand focus:ring-brand/30 cursor-pointer" />
              <span className="text-sm text-ink-secondary">Remember me for 30 days</span>
            </label>
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 text-xs text-ink-muted bg-surface-secondary dark:bg-dark-bg">or continue with</span>
            </div>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-ink-secondary">
            ...
            <Link to="/register" className="text-brand font-medium hover:underline">Sign up free</Link>
          </p>
        </motion.div>
      </div>
    </div>
